...
def main(argv):
    try:
        ...
        for batchIdx, batchDate in enumerate(batchDates):
            ...
            process_batch(config, batchIdx, batchDate, detectedSmells)
...
def add_to_smells_dataset(config, starting_date, detected_smells):
        dataframe = pd.DataFrame({
            ...
            'TF': [str(detected_smells.count('TF'))],
            'UI': [str(detected_smells.count('UI'))],
            'TC': [str(detected_smells.count('TC'))]
        })

        try:
            existing_data = pd.read_excel('./communitySmellsDataset.xlsx', sheet_name="dataset")
            merged_data = pd.concat([existing_data, dataframe], ignore_index=True)
            merged_data.to_excel('./communitySmellsDataset.xlsx', sheet_name="dataset", index=False)
        except FileNotFoundError:
            dataframe.to_excel('./communitySmellsDataset.xlsx', sheet_name="dataset", index=False)

def process_batch(config, batchIdx, batchDate, detectedSmells):
    result = pd.DataFrame(columns=['Index', 'StartingDate'])
    result["Index"] = batchIdx
    result["StartingDate"] = batchDate.strftime("%m/%d/%Y")

    for index, smell in enumerate(detectedSmells, start=1):
        smellName = "Smell" + str(index)
        result[smellName] = [smell, get_community_smell_name(smell)]

    add_to_smells_dataset(config, batchDate.strftime("%m/%d/%Y"), detectedSmells)
